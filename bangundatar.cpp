...
// Program  : Kalkulator Luas dan Keliling Bangun Datar
// Deskripsi: Program menghitung luas dan keliling berbagai
...
// 1. STRUCT — mendefinisikan entitas/objek Bangun Datar
struct BangunDatar {
    string nama;
    double luas;
    double keliling;
...
// 4. NAMESPACE — membungkus fungsi logika utama program
namespace GeometriCalc {

    const double PI = 3.14159265358979323846;

    
    // 6a. INLINE FUNCTION — fungsi pendek untuk format garis pemisah
    inline void cetakGaris() {
        cout << string(50, '=') << endl;
    }

    
    // 6b. INLINE FUNCTION — fungsi pendek untuk cetak judul section
    inline void cetakJudul(const string& judul) {
        cout << "  [ " << judul << " ]" << endl;
    }

    
    // 2. REFERENCES (&) — parameter dilewatkan via referensi
    //  Persegi 
    ...
    // 3. POINTER (*) — fungsi mencetak hasil melalui pointer
    ...
    // 5. CALLBACK FUNCTION — fungsi pemrosesan menerima fungsi lain
    ...
    // 7a. FUNCTION OVERLOADING — fungsi 'tampilInfo' dengan parameter berbeda
    ...
    // 6c. DEFAULT ARGUMENT — fungsi dengan nilai default satuan
...
} // end namespace GeometriCalc


// 7b. FUNCTION TEMPLATE (Opsional) — fungsi validasi input
//     yang bisa menerima berbagai tipe numerik
